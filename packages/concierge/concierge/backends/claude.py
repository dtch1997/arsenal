"""The Claude backend: the per-task worker process, running an Agent SDK
session detached from the daemon.
...
signal_blocked / signal_waiting / delegate are in-process SDK custom tools
here; the file-writing halves of blocked/waiting are shared with every other
backend via concierge.signals.

This is the default backend, behavior-identical to the original single-backend
worker.py (which now re-exports from here). See concierge.backends for the seam.
"""
...
from .. import delegation, signals
...
def _normalize(message) -> dict | None:
    if isinstance(message, SystemMessage) and message.subtype == "init":
        # backend on the init event for observability (issue #60): every
        # agent.jsonl declares which backend produced it
        return {"type": "system", "subtype": "init", "backend": BACKEND, **message.data}
...
def _blocked_tool(home: Home, tid: str):
    ...
    async def signal_blocked(args):
        signals.post_blocked(home, tid, str(args["question"]))
...
def _waiting_tool(home: Home, tid: str, cfg: dict, attempt: int):
    ...
    async def signal_waiting(args):
        signals.write_waiting(
            home, tid,
            until_shell=str(args["until_shell"]), note=str(args["note"]),
            attempt=attempt, timeout_minutes=args.get("timeout_minutes"),
    ...
    @tool("delegate",
          ...
          {"type": "object",
           "properties": {
               ...
               "backend": {"type": "string",
                           "description": "worker backend for the child: 'claude' (default, "
                                          "inherits yours) or 'codex' for a cheap line worker"},
               "base": {"type": "string",
    ...
    async def delegate(args):
        ...
        try:
            child = delegation.delegate_child(
                ...
                model=args.get("model"), backend=args.get("backend"),
                base=args.get("base"))
...
def main():
    ap = argparse.ArgumentParser(prog="python -m concierge.backends.claude")
